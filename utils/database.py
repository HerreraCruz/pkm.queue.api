...
async def execute_query_json(sql_template, params=None, needs_commit=False):
    ...
    try:
        ...
        if cursor.description:
            ...
            logger.info(f"Columas obtenidas: {columns}")
            for row in cursor.fetchall():
                procesed_row = [str(item) if isinstance(item, (bytes, bytearray)) else item for item in row]
                results.append(dict(zip(columns, procesed_row)))
        else:
        ...
        return json.dumps(results, default=str)  # Convert to JSON string
    
    except pyodbc.Error as e:
        logger.error(f"Error ejecutando la consulta (SQLSTATE:{e.args[0]}): {str(e)}")
        if conn and needs_commit:
            try:
                logger.warning("Realizando rollback debido a un error.")
                conn.rollback()
